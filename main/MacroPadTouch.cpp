...
#include "services/AsyncWebServerManager.hpp" 
...
static void app_event_handler(void* arg, esp_event_base_t event_base,
    ...
    // We can use a simple static flag to ensure the server is only started once.
    static bool server_started = false;

    if (event_base == APP_EVENTS) {
        if ((event_id == APP_EVENT_WIFI_STA_CONNECTED || event_id == APP_EVENT_WIFI_AP_STARTED) && !server_started) {
            
            ESP_LOGI(TAG_APP, "Network ready. Starting Async Web Server...");
        ...
        if (event_id == APP_EVENT_WIFI_DISCONNECTED) {
             // Logic for server management can be added here if needed, 
             // but typically the AsyncWebServer is kept running in a dedicated task.
        }
    }
}
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI("Main", "Initializing button configuration reader.");
    ButtonConfigReader configReader("/spiffs/btn_config.csv");
    
    configReader.loadAndRetryUntilComplete();
